...
const BscTeleportAgentMocks_AddressImpl = artifacts.require('contracts/mocks/BscTeleportAgentMocks.sol:AddressImpl');
const EthTeleportAgentMocks_AddressImpl = artifacts.require('contracts/mocks/EthTeleportAgentMocks.sol:AddressImpl');

contract('BscTeleportAgentMocks_AddressImpl', function (accounts) {
  shouldBehaveLikeAddress(BscTeleportAgentMocks_AddressImpl, accounts);
});

contract('EthTeleportAgentMocks_AddressImpl', function (accounts) {
  shouldBehaveLikeAddress(EthTeleportAgentMocks_AddressImpl, accounts);
});


function shouldBehaveLikeAddress(artifact, accounts) {
  const [ recipient, other ] = accounts;

  beforeEach(async function () {
    this.mock = await artifact.new();
  });
  ...
  describe('functionCallWithValue', function () {
    ...
    context('with non-zero value', function () {
      ...
      it('reverts when calling non-payable functions', async function () {
        ...
        await expectRevert(
          this.mock.functionCallWithValue(this.contractRecipient.address, abiEncodedCall, amount),
          "functionCallWithValue-mock-failed",
        );
      });
    });
  });
