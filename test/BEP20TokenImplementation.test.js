...
const ERC20Mock = artifacts.require('BEP20Mock');

contract('BEP20TokenImplementation', function (accounts) {
  ...
  beforeEach(async function () {
    ...
    await this.token.initialize(name, symbol, 18, initialHolder);
  ...
  it('has an owner', async function () {
    expect(await this.token.getOwner()).to.equal(initialHolder);
  });
  ...
  describe('_mint', function () {
    ...
    it('rejects a null account', async function () {
      await expectRevert(
        this.token.mintTo(ZERO_ADDRESS, amount), 'BEP20: mint to the zero address',
      );
    });

    describe('for a non zero account', function () {
      beforeEach('minting', async function () {
        const { logs } = await this.token.mintTo(recipient, amount);
        this.logs = logs;
      });
